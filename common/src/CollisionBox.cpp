 ...
 */
template<typename T>
bool CollisionBox<T>::ContactDetection(
    ...
    for(size_t i(0); i<3; ++i){
        ...
        else if(center2cp_local[i] < -_size[i]/2.) { inside_box = false; }
        // Distance from surface
        else{ err[i] = _size[i]/2. - std::abs(center2cp_local[i]); }
    }
    ...
    if(inside_box){
        if(err[2] < std::min(err[0], err[1])){ // Contact on Z surface
            ...
            if(center2cp_local[1] > 0.){
                // X -->X
                cp_frame.template block<3,1>(0,0) = _orientation.template block<3,1>(0,0);
                // -Z --> Y
                cp_frame.template block<3,1>(0,1) = -_orientation.template block<3,1>(0,2);
                // Y --> Z
                cp_frame.template block<3,1>(0,2) = _orientation.template block<3,1>(0,1);
            }else{
                // X --> X
                cp_frame.template block<3,1>(0,0) = _orientation.template block<3,1>(0,0);
                // Z --> Y
            ...
            if(center2cp_local[0] > 0.){
                // X -->X
                cp_frame.template block<3,1>(0,0) = _orientation.template block<3,1>(0,1);
                // -Z --> Y
                cp_frame.template block<3,1>(0,1) = -_orientation.template block<3,1>(0,2);
                // Y --> Z
                cp_frame.template block<3,1>(0,2) = _orientation.template block<3,1>(0,0);
            }else{
                // X -->X
                cp_frame.template block<3,1>(0,0) = _orientation.template block<3,1>(0,0);
                // -Z --> Y
                cp_frame.template block<3,1>(0,1) = _orientation.template block<3,1>(0,1);
                // Y --> Z
                cp_frame.template block<3,1>(0,2) = -_orientation.template block<3,1>(0,0);
            }
            penetration = -err[0];
