...
void ContactImpulse<T>::UpdateQdot(FBModelState<T> & state){
    ...
    if(CC::_nContact > 0){
        ...
        for(size_t i(0); i<CC::_nContact; ++i){
            ...
            //if(count%1000==0)pretty_print(CC::_model->_Jc[CC::_idx_list[i]], std::cout, "Jc");
        }
    }
}
...
void ContactImpulse<T>::_UpdateVelocity(DVec<T> & qdot){
    ...
    for(size_t i(0); i< CC::_nContact; ++i){
        // Lambda & Ainv*J'
        lambda_list_x[i] = CC::_model->applyTestForce(CC::_idx_list[i], 
                CC::_cp_frame_list[i].template block<3,1> (0, 0), AinvB_list_x[i]);
        lambda_list_x[i] = 1./lambda_list_x[i];

        lambda_list_y[i] = CC::_model->applyTestForce(CC::_idx_list[i], 
                CC::_cp_frame_list[i].template block<3,1> (0, 1), AinvB_list_y[i]);
        lambda_list_y[i] = 1./lambda_list_y[i];

        lambda_list_z[i] = CC::_model->applyTestForce(CC::_idx_list[i], 
                CC::_cp_frame_list[i].template block<3,1> (0, 2), AinvB_list_z[i]);
        ...
        if(cp_local_vel[2] < 0.){
            des_vel_list_z[i] = -CC::_cp_resti_list[i] * cp_local_vel[2] 
                -_penetration_recover_ratio * CC::_cp_penetration_list[i];
        }else {
            des_vel_list_z[i] = std::max(cp_local_vel[2], 
                    -_penetration_recover_ratio * CC::_cp_penetration_list[i]);
        }
