...
void ContactSpringDamper<T>::UpdateExternalForces(T K, T D, T dt){
    ...
    if(CC::_nContact > 0){
        ...
        for(size_t i(0); i<CC::_nContact; ++i ){
            CC::_model->_externalForces.at(
                    CC::_model->_gcParent[CC::_idx_list[i] ]) 
                += forceToSpatialForce(CC::_cp_force_list[CC::_idx_list[i]], CC::_cp_pos_list[i]);
       }
        static int count(0);
        ++count;
        if(count> 5){
...
void ContactSpringDamper<T>::_groundContactWithOffset(T K, T D) {
    ...
    for(size_t i = 0; i < CC::_nContact; i++) {
        ...
        Vec3<T> v = CC::_cp_frame_list[i].transpose() * CC::_model->_vGC[CC::_idx_list[i]];              // velocity in plane coordinates
        ...
        if(normalForce >0){
            CC::_cp_local_force_list[i][2] = normalForce;   // set the normal force. This is in the plane's coordinates for now
            ...
            Vec2<T> tangentialSpringForce = 
                K * zr * _tangentialDeflections[CC::_idx_list[i]]; // tangential force due to "spring"
            Vec2<T> tangentialForce = -tangentialSpringForce 
                - D * zr * deflectionRate[CC::_idx_list[i]]; // add damping to get total tangential

            // check for slipping:
            T slipForce = CC::_cp_mu_list[i] * normalForce;        // maximum force magnitude without slipping
