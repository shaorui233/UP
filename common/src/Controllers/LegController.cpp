...
/*!
 * Zero all leg commands.  This should be run *before* any control code, so if the control
 * code is confused and doesn't change the leg command, the legs won't remember the last command.
 */
template <typename T>
...
/*!
 * Set the leg to edamp.  This overwrites all command data and generates an emergency damp command
 * using the given gain. For the mini-cheetah, the edamp gain is Nm/(rad/s), and for the Cheetah 3 it is N/m.
 * You still must call updateCommand for this command to end up in the low-level command data!
 */
template <typename T>
...
/*!
 * Compute the position of the foot and its Jacobian.  This is done in the local leg coordinate system.
 * If J/p are NULL, the calculation will be skipped.
 */
template<typename T>
