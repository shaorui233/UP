...
void LinearKFPositionVelocityEstimator<T>::run() {
  ...
  Vec3<T> a = this->_stateEstimatorData.result->aWorld + g; // in old code, Rbod * se_acc + g
  std::cout << "A WORLD\n" << a << "\n";
  ...
  for(int i = 0; i < 4; i++) {
    ...
    Vec3<T> p_rel = ph + this->_stateEstimatorData.legControllerData[i].p;//hw_i->leg_controller->leg_datas[i].p; //
    Vec3<T> dp_rel = this->_stateEstimatorData.legControllerData[i].v;//hw_i->leg_controller->leg_datas[i].v;
    ...
    //printf("Trust %d: %.3f\n", i, trust);
    Q.block(qindex, qindex, 3, 3) = (T(1) + (T(1)-trust)*T(100))*Q.block(qindex, qindex, 3, 3);
    R.block(rindex1, rindex1, 3, 3) = 1*R.block(rindex1, rindex1, 3, 3);
    R.block(rindex2, rindex2, 3, 3) = (T(1) + (T(1)-trust)*100.0f)*R.block(rindex2, rindex2, 3, 3);
    R(rindex3, rindex3) = (T(1) + (T(1)-trust)*T(100))*R(rindex3, rindex3);
  ...
  for(int foot = 0; foot < 4; foot++) {
    for(int j = 0; j < 3; j++) {
      this->_stateEstimatorData.result->pFeetWorld[foot][j] = _xhat[6 + (foot*3) + j];
    }
  }
