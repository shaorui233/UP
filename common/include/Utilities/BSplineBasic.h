...
template<typename T, int DIM, int DEGREE, int NUM_MIDDLE, int CONST_LEVEL_INI, int CONST_LEVEL_FIN>
 ...
 BS_Basic():
  ...
  virtual bool SetParam(T* init, T* fin, T ** middle_pt, T fin_time){
    _CalcKnot(fin_time);
  ...
  virtual bool getCurvePoint(T u, T * ret){
    int _span;
 ...
 protected:
  ...
  inline T _Left( int i, int j, T u )  {
    return u - Knots_[i + 1 - j];
  }

  inline T _Right( int i, int j, T u ) {
    return Knots_[i + j] - u;
  }
