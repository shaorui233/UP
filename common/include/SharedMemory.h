/*! @file SharedMemory.h
 *  @brief Shared memory utilities for connecting the simulator program to the robot program
 *
 *
 */
...
public:
  ...
  /*!
   * If semaphore is unitialized, initialize it and set its value.  This can be called as many times as you want safely.
   * It must be called at least once.  Only one process needs to call this, even if it is used
...
/*!
 * A container class for an object which is stored in shared memory.  This object can then be viewed in multiple
 * processes or programs.  Note that there is significant overhead when creating a shared memory object, so it is
 * recommended that two programs that communicate should have one single large SharedMemoryObject instead of many
 * small ones.
 *
 * A name string is used to identify shared objects across different programs
 *
 * Before a shared memory object can be used, you must either allocate new memory, or connect it to an existing
 * shared memory object.
 *
 * Creating/deleting the memory can be done with createNew/closeNew.
 * Viewing an existing object allocated with createNew can be done with attach/detach
 *
...
public:
   ...
   * If allowOverwrite is true, and there's already an object with this name, the old object is overwritten
  ...
  void attach(const std::string &name) {
    ...
    if((size_t)s.st_size != _size) {
      printf("[ERROR] SharedMemoryObject::attach(%s) on something that was incorrectly "
  ...
  /*!
   * Free memory associated with the current open shared memory object.  The object could have been opened with
   * either attach or createNew.  After calling this, no process can use this shared object
   */
  void closeNew() {
    assert(_data);
    // first, unmap
  ...
  /*!
   * Close this view of the currently opened shared memory object. The object can be opened with either
   * attach or createNew.  After calling this, this process can no longer use this shared object,
   * but other processes still can.
   */
  void detach() {
    assert(_data);
    // first, unmap
