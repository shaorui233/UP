 ...
 *  @brief Nonlinear friction and collision model used in the simulator for
 * ground contact
 *
 *  The collision model is based on the gcontact.m file in spatial_v2 and
 * modified by Pat for codegen This uses the model in "Modelling the Contact
 * Between a Rolling Sphere and a Compliant Ground Plane" by Azad and
 * Featherstone.  It assumes a point contact.
 *
 */
...
/*!
 * Run the ground contact model for a single collision plane on a list of ground
 * contact points The ground is allowed to deform in the tangential direction,
 * but not the normal direction. The ground also "remembers" its deformation
 * between separate contact events. (however it does spring back pretty quickly)
 * @param _pGC List of ground contact point locations.
 * @param _vGC List of ground contact point velocities
 * @param _deflections List of deflection state variables.  This is updated by
 * this model
 ...
 */
template <typename T>
void groundContactModelWithOffset(vectorAligned<Vec3<T>>& _pGC,
                                  vectorAligned<Vec3<T>>& _vGC,
                                  vectorAligned<Vec2<T>>& _deflections,
                                  vectorAligned<Vec3<T>>& _forces, T K, T D,
                                  T mu, T dt, Mat6<T>& X) {
  ...
  for (size_t i = 0; i < nPt; i++) {
    ...
    Vec2<T> deflection =
        _deflections[i];  // the deflection of the ground from previously
    ...
    Vec2<T> deflectionRate =
        (-K / D) * deflection.template topLeftCorner<2, 1>();

    if (inContact) {
      _forces[i][2] = normalForce;  // set the normal force. This is in the
                                    // plane's coordinates for now
      ...
      Vec2<T> tangentialSpringForce =
          K * zr * deflection;  // tangential force due to "spring"
      Vec2<T> tangentialForce =
          -tangentialSpringForce -
          D * zr * deflectionRate;  // add damping to get total tangential

      // check for slipping:
      T slipForce =
          mu * normalForce;  // maximum force magnitude without slipping
 ...
 * The ground is allowed the deform in the tangential direction, but not the
 * normal direction. The ground also "remembers" its deformation between
 * separate contact events. (however it does spring back pretty quickly)
 * @param _pGC List of ground contact point locations.
 * @param _vGC List of ground contact point velocities
 * @param _deflections List of deflection state variables.  This is updated by
 * this model
 ...
 */
template <typename T>
void groundContactModel(vectorAligned<Vec3<T>>& _pGC,
                        vectorAligned<Vec3<T>>& _vGC,
                        vectorAligned<Vec2<T>>& _deflections,
                        vectorAligned<Vec3<T>>& _forces, T K, T D, T mu, T dt) {
  Mat6<T> eye = Mat6<T>::Identity();
  groundContactModelWithOffset(_pGC, _vGC, _deflections, _forces, K, D, mu, dt,
                               eye);
}
