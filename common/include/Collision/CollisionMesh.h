...
class CollisionMesh : public Collision<T>{
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
       ...
       * @param left_corner_loc : global location of left bottom edge of the height map
       * @param height_map : Height map of mesh
       */
      CollisionMesh(const T & mu, 
          const T & restitution, const T & grid,
          const Vec3<T> & left_corner_loc, const DMat<T> & height_map ) :
        Collision<T>(mu, restitution),
        _left_corner_loc(left_corner_loc), _height_map(height_map), _grid(grid){
