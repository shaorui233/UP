...
spine_boardcontrol::spine_boardcontrol(ti_flt side_sign, s32 board)
{
...
void spine_boardcontrol::reset_spine_board_data()
{
    if(data == nullptr)
    {
        printf("[ERROR: SPINE board] reset_spine_board_data called when cheetahlcm_spi_data_t* was null\n");
        return;
    }
...
void spine_boardcontrol::reset_spine_board_command()
{
    if(cmd == nullptr)
    {
        printf("[ERROR: SPINE board] reset_spine_board_command called when cheetahlcm_spi_command_t* was null\n");
        return;
    }
...
void spine_boardcontrol::run_spine_board_iteration()
{
    iter_counter++;
    if(cmd == nullptr || data == nullptr)
    {
        printf("[ERROR: SPINE board] run_spine_board_iteration called with null command or data!\n");
        torque_out[0] = 0.f;
        torque_out[1] = 0.f;
        torque_out[2] = 0.f;
        return;
    ...
    if(data->q_abad[board_num] > q_limit_p[0])
    {
        torque_out[0] = kp_softstop*(q_limit_p[0] - data->q_abad[board_num])
                        -kd_softstop*(data->qd_abad[board_num]) + cmd->tau_abad_ff[board_num];
    }
    ...
    if(data->q_hip[board_num] > q_limit_p[1])
    {
        torque_out[1] = kp_softstop*(q_limit_p[1] - data->q_hip[board_num])
                        -kd_softstop*(data->qd_hip[board_num]) + cmd->tau_hip_ff[board_num];  
    }
    ...
    const ti_flt* torque_limits = disabled_torque;
