...
void RobotInterface::sendControlParameter(const std::string &name, ControlParameterValue value,
                                          ControlParameterValueKind kind) {
  if(_pendingControlParameterSend)
  {
    printf("[ERROR] trying to send control parameter while a send is in progress, ignoring!\n");
    return;
  }
  ...
  for(int iteration = 0; iteration < TIMES_TO_RESEND_CONTROL_PARAM;) {
    ...
    // message data
    _parameter_request_lcmt.requestKind = (s8)ControlParameterRequestKind::SET_PARAM_BY_NAME;
    ...
    if(_lcmCV.wait_for(lock, 100ms) == std::cv_status::no_timeout) {
      ...
      if(_waitingForLcmResponse || _lcmResponseBad) {
        printf("[RobotInterface] Failed to send parameter %s (iter %d) "
      ...
      _waitingForLcmResponse = false;
      // fail!
      printf("[RobotInterface] Failed to send parameter %s (iter %d timed out), trying again...\n",
...
void RobotInterface::handleControlParameter(const lcm::ReceiveBuffer *rbuf, const std::string &chan,
                                            const control_parameter_respones_lcmt *msg) {
  (void)rbuf;
  (void)chan;
  if(!_waitingForLcmResponse) {
    printf("[RobotInterface] Got a control parameter response when we weren't expecting one, ignoring it!\n");
    _lcmResponseBad = true;
    return;
  }
