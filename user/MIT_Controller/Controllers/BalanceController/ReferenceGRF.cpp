...
ReferenceGRF::ReferenceGRF() :  QProblemObj_qpOASES(NUM_VARIABLES_QP_DES,NUM_CONSTRAINTS_QP_DES)
{
   ...
   A_eigen.resize(NUM_CONSTRAINTS_QP_DES,NUM_VARIABLES_QP_DES);
...
void ReferenceGRF::calc_A_qpOASES()
{
   Eigen::MatrixXd IDNTY;IDNTY.setIdentity(NUM_CONSTRAINTS_QP_DES,NUM_VARIABLES_QP_DES);
   copy_Eigen_to_real_t(A_qpOASES, IDNTY, NUM_CONSTRAINTS_QP_DES, NUM_VARIABLES_QP_DES); 
}

void ReferenceGRF::calc_g_qpOASES()
{  
   // Use LQR matrices to compute the QP cost vector g
   Eigen::VectorXd f_avg;f_avg.resize(NUM_VARIABLES_QP_DES,1);
   f_avg << 100.5525, 100.5525, 100.5525, 100.5525;
   g_eigen = -2*A_control.transpose()*b_control;
   g_eigen+= -2*alpha_control*f_avg;
...
void ReferenceGRF::calc_lb_ub_qpOASES()
{
   for(int i = 0; i < NUM_VARIABLES_QP_DES; i++)
   {
      lb_qpOASES[i] = contact_state(i)*minNormalForces_feet(i);
      ub_qpOASES[i] = contact_state(i)*maxNormalForces_feet(i);    
   }

}

void ReferenceGRF::calc_lbA_ubA_qpOASES()
{
   for(int i = 0; i < NUM_CONSTRAINTS_QP_DES; i++)
   {
      lbA_qpOASES[i]   = contact_state(i)*NEGATIVE_NUMBER_DES;      
      ubA_qpOASES[i]   = contact_state(i)*POSITIVE_NUMBER_DES;
   }
}
