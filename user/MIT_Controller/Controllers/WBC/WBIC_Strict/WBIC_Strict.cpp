#include "WBIC_Strict.hpp"
#include <Utilities/Timer.h>
#include <eigen3/Eigen/LU>
#include <eigen3/Eigen/SVD>

  template <typename T>
WBIC_Strict<T>::WBIC_Strict(size_t num_qdot, const std::vector<ContactSpec<T>*>* contact_list,
...
void WBIC_Strict<T>::MakeTorque(DVec<T>& cmd, void* extra_input) {
  if (!WB::b_updatesetting_) {
    printf("[Wanning] WBIC_Strict setting is not done\n");
  }
  if (extra_input) _data = static_cast<WBIC_Strict_ExtraData<T>*>(extra_input);
...
template <typename T>
void WBIC_Strict<T>::_SetEqualityConstraint(const DVec<T>& qddot) {
  if (_dim_rf > 0) {
  ...
  // pretty_print(_dyn_CE, std::cout, "WBIC_Strict: CE");
  // pretty_print(_dyn_ce0, std::cout, "WBIC_Strict: ce0");
}

template <typename T>
void WBIC_Strict<T>::_SetInEqualityConstraint() {
  ...
  // pretty_print(_dyn_CI, std::cout, "WBIC_Strict: CI");
  // pretty_print(_dyn_ci0, std::cout, "WBIC_Strict: ci0");
}

template <typename T>
void WBIC_Strict<T>::_ContactBuilding() {
  DMat<T> Uf;
  DVec<T> Uf_ieq_vec;
  // Initial
  DMat<T> Jc;
  ...
  //pretty_print(_Fr_des, std::cout, "[WBIC_Strict] Fr des");
  // pretty_print(_Jc, std::cout, "[WBIC_Strict] Jc");
  // pretty_print(_JcDotQdot, std::cout, "[WBIC_Strict] JcDot Qdot");
  // pretty_print(_Uf, std::cout, "[WBIC_Strict] Uf");
}

template <typename T>
void WBIC_Strict<T>::_GetSolution(const DVec<T>& qddot, DVec<T>& cmd) {
  DVec<T> tot_tau;
  if (_dim_rf > 0) {
...
template <typename T>
void WBIC_Strict<T>::UpdateSetting(const DMat<T>& A, const DMat<T>& Ainv,
...
template <typename T>
void WBIC_Strict<T>::_SetOptimizationSize() {
  // Dimension
  _dim_rf = 0;
...
template class WBIC_Strict<double>;
template class WBIC_Strict<float>;
