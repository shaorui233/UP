...
//Matrix<fpt,Dynamic,Dynamic> power_fast(Matrix<fpt,Dynamic,Dynamic> m, s16 p)
//{
//    
//}
...
void c2qp(Matrix<fpt,13,13> Ac, Matrix<fpt,13,12> Bc,fpt dt,s16 horizon)
{
    ...
    ABc = dt*ABc;
    //ABc *= dt;
    ////TODO check this out in more detail
    expmm = ABc.exp();
    ...
    cout<<"AQP:\n"<<A_qp<<"\nBQP:\n"<<B_qp<<endl;
#endif
    //cout<<"BQP sum: "<<B_qp.sum()<<" trace: "<<B_qp.trace()<<endl;
    //if(B_qp(2,3) < 23) printf("derp.\n");
 ...
 //   printf("%d\n",dtt);
    ...
    if(1==1)
    {
        ...
        for(int i = 0; i < num_variables; i++)
        {
            if(var_elim[i])
            {
                //if(q_soln[i] > 1.f || q_soln[i] < -1.f)
                //    printf("%f ",q_soln[i]);
                q_soln[i] = 0.0f;
            }
            else
            {
                //fpt asdf = q_soln[i] - q_red[vc];
                //if(asdf > 1.f || asdf < -1.f)
                    //printf("%f ",q_soln[i] - q_red[vc]);
                q_soln[i] = q_red[vc];
                vc++;
            }
        }
   ...
   //     for(int c = 0; c < 108; c++)
   //     {
   //         printf("%f, ",H_qpoases[r*108 + c]);
   //     }
   //     printf("\n");
   // }

   // printf("g\n");
   // for(int i = 0; i < setup->horizon*12; i++)
   //     printf("%.3f ",g_qpoases[i]);
   // printf("\n");

   // printf("lb\n");
   // for(int i = 0; i < setup->horizon*20; i++)
   //     printf("%.6f ",lb_qpoases[i]);
   // printf("\n");

   // printf("ub\n");
   // for(int i = 0; i < setup->horizon*20; i++)
   //     printf("%.3f ",ub_qpoases[i]);
   // printf("\n");
    
   // printf("qsoln:\n");
   // for(int j = 0; j < setup->horizon; j++)
   // {
   //     for(int i = 0; i < 12; i++)
   //         printf("%.3f ",q_soln[i + 12*j]);
   //     printf("\n");
   // }
