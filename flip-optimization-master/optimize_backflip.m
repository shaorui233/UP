...
if ~(exist('symbolic_done','var'))
    ...
    tic;
    params = get_robot_params(0);  % robot masses, lengths...
...
N = 88 * res; % number of timesteps
dt = 0.01 / res; % time between timesteps
double_contact = 24*res; % timesteps spent with both feet on ground
single_contact = 20*res; % timesteps spent with single foot on ground
...
opti.subject_to(q(2,N) >= .25); % end position
