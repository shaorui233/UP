/*! @file SimCheckerBoard.cpp
 *  @brief 3D Plane with SimCheckerBoard Pattern for graphics
 *
 */

#include "SimCheckerBoard.hpp"
#include <stdio.h>
 ...
 */
SimCheckerBoard::SimCheckerBoard(float xSize, float ySize, size_t xSquares, size_t ySquares) {
  _size[0] = xSize;
  _size[1] = ySize;
  _squares[0] = xSquares;
  _squares[1] = ySquares;
 ...
 */
void SimCheckerBoard::computeVertices(std::vector<float>& vertices, std::vector<float>& normals, std::vector<float>& colors) {
