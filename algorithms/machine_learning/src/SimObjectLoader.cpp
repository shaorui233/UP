...
void SimObjectLoader::load(const char *filename, std::vector<float> &positions, std::vector<float> &normals) {
  ifstream inf;
...
int SimObjectLoader::getIndexCount() {
  return (int) Faces.size() * 3;
}

int SimObjectLoader::getVertCount() {
  return (int) Positions.size();
}

const unsigned int *SimObjectLoader::getFaces() {
  return (const unsigned int *) &Faces[0];
}

float *SimObjectLoader::getPositions() {
  return positions_jdc;
}

float *SimObjectLoader::getNormals() {
  return normals_jdc;
}

int SimObjectLoader::getTexCoordLayers() {
  return TexCoordLayers;
}

const float *SimObjectLoader::getTexCoords(int multiTexCoordLayer) {
