 ...
 */
SimGraphics3D::SimGraphics3D(QWidget *parent) : 
    QOpenGLWidget(parent), _animating(false), _colorArrayProgram(0),
 ...
 */
size_t SimGraphics3D::setupCheetah3() {
 ...
 */
size_t SimGraphics3D::setupMiniCheetah() {
...
void SimGraphics3D::updateCameraMatrix() {
...
void SimGraphics3D::initializeGL() {
  std::cout << "[SimGraphics3D] Initialize OpenGL...\n";
...
void SimGraphics3D::mousePressEvent(QMouseEvent *event) {
  _orbiting = true;
...
void SimGraphics3D::wheelEvent(QWheelEvent *e) {
  if (e->angleDelta().y() > 0) {
    if (_zoom > .1)
      _zoom = 0.8f * _zoom;
  } else {
...
void SimGraphics3D::keyPressEvent(QKeyEvent *e) {
...
void SimGraphics3D::keyReleaseEvent(QKeyEvent *e) {
  if (e->key() == Qt::Key_Control) {
    _targetSpeed /= .5f;
  } else if (e->key() == Qt::Key_Shift) {
    _targetSpeed /= 2.f;
 ...
 */
void SimGraphics3D::setAnimating(bool animating) {
  _animating = animating;
}


void SimGraphics3D::renderDrawlist() {
  // reload if needed
...
void SimGraphics3D::configOpenGLPass(int pass) {
  switch (pass) {
    case 0:
      // clear screen
...
void SimGraphics3D::_MeshObstacleDrawing(){
...
void SimGraphics3D::getHeightColor(const double & h, float& r, float& g, float&b){
...
void SimGraphics3D::_SetRGBHeight(const double & h, const double & step, const int & idx,
...
void SimGraphics3D::_BoxObstacleDrawing() {
  glLoadMatrixf(_cameraMatrix.data());
  glPushAttrib(GL_LIGHTING_BIT);
  glDisable(GL_LIGHTING);
...
void SimGraphics3D::_DrawBox(double depth, double width, double height) {
  double x = depth / 2.0;
  double y = width / 2.0;
  double z = height / 2.0;
...
void SimGraphics3D::_Additional_Drawing(int pass) {
  glLoadMatrixf(_cameraMatrix.data());
  glPushAttrib(GL_LIGHTING_BIT);
  glDisable(GL_LIGHTING);
...
void SimGraphics3D::_DrawContactForce() {
  glLineWidth(2.0);
  //double scale(0.02);
  double scale(0.0025);
...
void SimGraphics3D::_DrawContactPoint() {
  glPointSize(5);
...
void SimGraphics3D::_rotateZtoDirection(const Vec3<float> &direction) {
...
void SimGraphics3D::_drawSphere(SphereVisualization &sphere) {
...
void SimGraphics3D::_drawCone(ConeVisualization &cone) {
  static GLUquadric *quad = gluNewQuadric();
...
void SimGraphics3D::_drawBlock(BlockVisualization &box) {
  glPushMatrix();
...
void SimGraphics3D::_drawArrow(ArrowVisualization &arrow) {
  _setColor(arrow.color);
  _drawArrow(arrow.base_position, arrow.direction,
             arrow.shaft_width, arrow.head_width, arrow.head_length);
}

void SimGraphics3D::_drawArrow(const Vec3<float> &position, const Vec3<float> &direction, float lineWidth, float headWidth,
                            float headLength) {
