...
def env_cheetah_step(action, state, time_step):
#    delta_jpos_knee_command = (float(action)-5)/50
    jpos_knee_command = (float(action)-5)/20+1.715
...
#        print i
        ...
        in_jpos[0] = -0.03
        in_jpos[1] = -0.79
#        in_jpos[2] = state[0,8] + delta_jpos_knee_command #1.715
        in_jpos[2] = jpos_knee_command #1.715

        in_jpos[3] = 0.03
        in_jpos[4] = -0.72
        in_jpos[5] = jpos_knee_command #1.715
#        in_jpos[5] = state[0,11] + delta_jpos_knee_command #1.715

        in_jpos[6] = -0.03
        in_jpos[7] = -0.79
        in_jpos[8] = jpos_knee_command #1.715
#        in_jpos[8] = state[0,14] + delta_jpos_knee_command #1.715

        in_jpos[9] = 0.03
        in_jpos[10] = -0.72
        in_jpos[11] = jpos_knee_command #1.715
#        in_jpos[11] = state[0,17] + delta_jpos_knee_command #1.715
    ...
    done = termination_flag(state_next, time_step)
    return state_next, reward, done

def cheetah_reinforcement_learning():
    observation_space = 36
    action_space = 11 # 11 discrete value for delta_jpos_command
    ...
    while True:
        ...
        while True:
            ...
            state_next, reward, terminal = env_cheetah_step(action, state, step)
...
# replay the trained network
    ...
    while True:
        step += 1
        action = dqn_solver.act(state)
        state, reward, terminal = env_cheetah_step(action, state, step)
        state = np.reshape(state, [1, observation_space])
        if terminal:
