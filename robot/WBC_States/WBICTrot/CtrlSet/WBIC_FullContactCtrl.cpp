...
WBIC_FullContactCtrl<T>::WBIC_FullContactCtrl(
    WBICTrotTest<T>* trot_test, const FloatingBaseModel<T>* robot):Controller<T>(robot),
  ...
  _ctrl_start_time(0.)
{
  ...
  wbic_ = new WBIC<T>(cheetah::dim_config, &(Ctrl::_contact_list), &(Ctrl::_task_list));
...
void WBIC_FullContactCtrl<T>::_task_setup(){
  ...
  kin_wbc_->FindConfiguration(_sp->_Q,
      ...
      _trot_test->_des_jpos, _trot_test->_des_jvel, _trot_test->_des_jacc);
}
...
template <typename T>
void WBIC_FullContactCtrl<T>::CtrlInitialization(const std::string & category_name){
  (void)category_name;
}
