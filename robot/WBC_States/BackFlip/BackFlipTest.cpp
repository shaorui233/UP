...
template <typename T>
BackFlipTest<T>::BackFlipTest(FloatingBaseModel<T>* robot, const RobotType & type):
    Test<T>(robot, type){
...
void BackFlipTest<T>::_SettingParameter(){
    ...
    while(iter < Test<T>::_state_list.end()){
        if(Test<T>::_robot_type == RobotType::CHEETAH_3){
            (*iter)->SetTestParameter(CheetahConfigPath"TEST_backflip_cheetah3.yaml");
            ...
            (*iter)->SetTestParameter(CheetahConfigPath"TEST_backflip_mini_cheetah.yaml");
