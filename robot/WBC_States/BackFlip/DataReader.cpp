...
void DataReader::load_control_plan(const char *filename)
{
    ...
    {
        printf("[Backflip DataReader] Error: file size isn't divisible by size of float!\n");
    }
...
float* DataReader::get_initial_configuration()
{
    if(!plan_loaded)
    {
        printf("[Backflip DataReader] Error: get_initial_configuration called without a plan!\n");
        return nullptr;
    }
...
float* DataReader::get_plan_at_time(int timestep)
{
    if(!plan_loaded)
    {
        printf("[Backflip DataReader] Error: get_plan_at_time called without a plan!\n");
        return nullptr;
    }
