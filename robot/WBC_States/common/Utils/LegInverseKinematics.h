...
class SplineOptimizer {
 public:
  ...
  static double MinAcc(const std::vector<double>& x, std::vector<double>& grad,
                       void* d_) {
    (void)grad;
    (void)d_;
  ...
  static void BoundAcceleration(unsigned m, double* result, unsigned n,
    ...
    for (unsigned int k(0); k < DIM; ++k)
      tester->_fin_opt[DIM * 2 + k] = x[k + DIM];

    tester->_spline.SetParam(tester->_ini, tester->_fin_opt, tester->_mid_pt,
                             tester->_fin_time);
  ...
  void setParam(T* ini, T* fin, T** middle_pt, T fin_time) {
    ...
    for (size_t i(0); i < NUM_MIDDLE; ++i)
      for (size_t j(0); j < DIM; ++j) _mid_pt[i][j] = middle_pt[i][j];
    ...
    for (int i(0); i < num_acc_check_pt; ++i) {
      tol_ieq[i] = 1e-4;
    }
    test->add_inequality_mconstraint(SplineOptimizer::BoundAcceleration, this,
                                     tol_ieq);
  ...
  void getPos(T time, T* pos) { _spline.getCurvePoint(time, pos); }
  void getVel(T time, T* vel) { _spline.getCurveDerPoint(time, 1, vel); }
  void getAcc(T time, T* acc) { _spline.getCurveDerPoint(time, 2, acc); }
