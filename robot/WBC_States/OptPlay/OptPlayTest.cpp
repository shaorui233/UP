...
void OptPlayTest<T>::_UpdateExtraData(Cheetah_Extra_Data<T> * ext_data){
    ...
    for(size_t i(_sp->_num_step); i<num_planned_foot_step; ++i){
        ext_data->loc_x[2*i] = inter->_foot_step_list[i][0]; 
        ext_data->loc_x[2*i + 1] = inter->_foot_step_list[i][3]; 

        ext_data->loc_y[2*i] = inter->_foot_step_list[i][1]; 
        ext_data->loc_y[2*i + 1] = inter->_foot_step_list[i][4]; 
        
        ext_data->loc_z[2*i] = inter->_foot_step_list[i][2]; 
        ext_data->loc_z[2*i + 1] = inter->_foot_step_list[i][5]; 
     }
    ext_data->num_step = (num_planned_foot_step - _sp->_num_step);
}
