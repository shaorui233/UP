...
void KinBoundingCtrl<T>::_StatusCheck(){
  ...
  //T K_time(0.7);
  //static bool b_jump(true);
  static bool b_jump(false);
  static bool b_jump_front(false);
  if(_b_front_swing && (_front_time > 0.5 * _swing_time)){
    // Check Contact && TEST
    //if(_b_front_contact){
    if((_front_time > (0.5*_default_gait_period - 2.*Test<T>::dt)) && _b_first_stance){
      _b_front_swing = false;
      _front_start_time = _sp->_curr_time;
      _front_previous_swing = _front_time;
      //_front_current_stance = 
      //_stance_time - 
      //K_time* (_front_previous_stance + _front_previous_swing - _default_gait_period);
      ...
      _b_first_stance = false;
    } else if(_front_time > (_swing_time - 2.0*Test<T>::dt)){
      _b_front_swing = false;
      _front_start_time = _sp->_curr_time;
      _front_previous_swing = _front_time;
      //_front_current_stance = 
  ...
  if(_b_hind_swing && (_hind_time > 0.5 * _swing_time)){
    // Check Contact && TEST
    //if(_b_hind_contact){
    if(_hind_time > (_swing_time - 2.0*Test<T>::dt)){ // Switch to stance
      _b_hind_swing = false;
      _hind_start_time = _sp->_curr_time;
      _hind_previous_swing = _hind_time;
      //_hind_current_stance = 
      //_stance_time - 
      //K_time* (_hind_previous_stance + _hind_previous_swing - _default_gait_period);
      //K_time* (_hind_previous_stance + _aerial_duration - 0.5*_default_gait_period);
      //K_time* (_front_previous_stance + _aerial_duration - 0.5*_default_gait_period);
      //printf("front, hind swing: %d, %d\n", _b_front_swing, _b_hind_swing);
      //printf("front time, hind time: %f, %f\n", _front_time, _hind_time); 
      ...
      if((_sp->_curr_time > 4.) && b_jump_front){
        apex *= 7.5;
        b_jump_front = false;
      }
  ...
  template <typename T>
    void KinBoundingCtrl<T>::SetTestParameter(const std::string & test_file){
      ...
      //}
      //for(size_t i(0); i<tmp_vec.size(); ++i){ 
      //((LocalPosTask<T>*)_fl_foot_local_task)->_Kd[i] = tmp_vec[i]; 
      //}
      //for(size_t i(0); i<tmp_vec.size(); ++i){ 
      ...
      //for(size_t i(0); i<tmp_vec.size(); ++i){ 
      //((LocalPosTask<T>*)_hl_foot_local_task)->_Kd[i] = tmp_vec[i]; 
      //}
