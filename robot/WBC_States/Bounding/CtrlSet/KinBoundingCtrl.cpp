...
void KinBoundingCtrl<T>::_ContactUpdate(){
  ...
  if(_b_front_swing && (_front_time > 0.6*_swing_time)){
    if(front_knee_vel_diff > _contact_vel_threshold || (_front_time > 3.5*_swing_time) ){ 
  ...
  if(_b_hind_swing && (_hind_time > 0.6*_swing_time)){
    if((hind_knee_vel_diff > _contact_vel_threshold) || (_hind_time > 3.5*_swing_time)){ 
...
void KinBoundingCtrl<T>::_StatusCheck(){
  ...
  if(fabs(_bounding_test->_body_vel[0]) * _nominal_stance_time > _step_length_lim){
  ...
  if( _b_front_swing && (_front_time > 0.5 * _swing_time) ){
    ...
    if(_b_front_contact_est){
      ...
      adjust_time = _K_time* (_hind_previous_stance + _aerial_duration - 0.5*_gait_period);
  ...
  if(_b_hind_swing && (_hind_time > 0.5 * _swing_time)){
    ...
    if(_b_hind_contact_est){
      ...
      adjust_time = _K_time* (_front_previous_stance + _aerial_duration - 0.5*_gait_period);
      adjust_time = coerce<T>(adjust_time, 0., 0.04); 
      ...
      _ini_hind_body = 
        0.5*Ctrl::_robot_sys->_pGC[linkID::HR_abd] 
...
void KinBoundingCtrl<T>::_body_task_setup(){
  ...
  DVec<T> vel_des(2); vel_des.setZero();
  DVec<T> acc_des(2); acc_des.setZero();
