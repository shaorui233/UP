...
void KinBoundingCtrl<T>::FirstVisit(){
  ...
  T apex = _total_mass * 9.81 * (_swing_time + _nominal_stance_time)/(2. * 2.0*0.7*_nominal_stance_time);
...
void KinBoundingCtrl<T>::CtrlInitialization(const std::string & category_name){
  ...
  _param_handler->getValue<T>(category_name, "contact_vel_threshold", _contact_vel_threshold);
