...
bool KinWBC<T>::FindConfiguration(
  ...
  for (size_t i(1); i<task_list.size(); ++i){
    ...
    qddot = prev_qddot + JtPre_pinv * (task->getDesAcc()- JtDotQdot - Jt*prev_qddot);
...
template <typename T>
void KinWBC<T>::_BuildProjectionMatrix(const DMat<T> & J, 
    DMat<T> & N){
  DMat<T> J_pinv;
...
template <typename T>
void KinWBC<T>::_PseudoInverse(const DMat<T> J, 
    DMat<T> & Jinv){
