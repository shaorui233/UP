...
template<typename T> 
WBIC<T>::WBIC(size_t num_qdot): 
    WBC<T>(num_qdot), _dim_floating(6){

    _Sf = DMat<T>::Zero(_dim_floating, WB::num_qdot_);
    _Sf.block(0,0, _dim_floating, _dim_floating).setIdentity();
...
void WBIC<T>::_SetOptimizationSize(){
    ...
    for(size_t i(0); i<(_data->_contact_list).size(); ++i){
        _dim_rf += (_data->_contact_list)[i]->getDim();
        _dim_Uf += (_data->_contact_list)[i]->getDimRFConstraint();
    }
...
template<typename T> 
void WBIC<T>::MakeTorque(
        DVec<T> & cmd,
        void* extra_input){

    if(!WB::b_updatesetting_) { printf("[Wanning] WBIC setting is not done\n"); }
    ...
    for(size_t i(0); i<_data->_task_list.size(); ++i){
        task = _data->_task_list[i];
...
void WBIC<T>::_SetEqualityConstraint(const DVec<T> & qddot){
    if(_dim_rf > 0){
        _dyn_CE.block(0, 0, _dim_eq_cstr, _dim_floating) = _eye_floating;
        _dyn_CE.block(0, _dim_floating, _dim_eq_cstr, _dim_rf) = -_Sf * _Jc.transpose();
        _dyn_ce0 = -_Sf *(WB::A_*qddot + WB::cori_ + WB::grav_ - _Jc.transpose() * _data->_Fr_des);
    }else {
        _dyn_CE.block(0, 0, _dim_eq_cstr, _dim_floating) = _eye_floating;
        _dyn_ce0 = -_Sf *(WB::A_*qddot + WB::cori_ + WB::grav_);
    }
...
void WBIC<T>::_SetInEqualityConstraint(){
    ...
    // pretty_print(dyn_CI, std::cout, "WBIC: CI");
    // pretty_print(dyn_ci0, std::cout, "WBIC: ci0");
}
...
void WBIC<T>::_ContactBuilding(){
    ...
    _data->_contact_list[0]->getContactJacobian(Jc);
    _data->_contact_list[0]->getJcDotQdot(JcDotQdot);
    _data->_contact_list[0]->getRFConstraintMtx(Uf);
    _data->_contact_list[0]->getRFConstraintVec(Uf_ieq_vec);
    
    dim_accumul_rf = _data->_contact_list[0]->getDim();
    dim_accumul_uf = _data->_contact_list[0]->getDimRFConstraint();
    ...
    for(size_t i(1); i < _data->_contact_list.size(); ++i){
        _data->_contact_list[i]->getContactJacobian(Jc);
        _data->_contact_list[i]->getJcDotQdot(JcDotQdot);

        dim_new_rf = _data->_contact_list[i]->getDim();
        dim_new_uf = _data->_contact_list[i]->getDimRFConstraint();
        ...
        // Uf
        _data->_contact_list[i]->getRFConstraintMtx(Uf);
        _Uf.block(dim_accumul_uf, dim_accumul_rf, dim_new_uf, dim_new_rf) = Uf;

        // Uf inequality vector
        _data->_contact_list[i]->getRFConstraintVec(Uf_ieq_vec);
...
void WBIC<T>::_GetSolution(const DVec<T> & qddot, DVec<T> & cmd){
    ...
    if(_dim_rf > 0){
        ...
        // get Reaction forces
        for(size_t i(0); i<_dim_rf; ++i) _data->_Fr[i] = z[i + _dim_floating];
...
template<typename T> 
void WBIC<T>::UpdateSetting(const DMat<T> & A,
        const DMat<T> & Ainv,
        const DVec<T> & cori,
        const DVec<T> & grav,
        void* extra_setting){
