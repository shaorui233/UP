...
void BalanceController::updateProblemData(   double* xfb_in,
                           double* p_feet_in,
                           double* p_des,
                           double* p_act, 
                           double* v_des, 
...
{   
   ...
   // Compute controller matrices. Must call these before calculating H_qp and g_qp
   calc_PDcontrol();
   update_A_control();
...
void BalanceController::solveQP_nonThreaded(double* xOpt)
{
  ...
  copy_real_t_to_Eigen(xOpt_eigen, xOpt_qpOASES, 12);
  // copy_real_t_to_Eigen(yOptPrev, yOpt_qpOASES, NUM_VARIABLES_QP+NUM_CONSTRAINTS_QP);
...
void BalanceController::calc_H_qpOASES()
{
   ...
   H_eigen = 2*(A_control.transpose()*S_control*A_control + (alpha_control + 1e-3)*W_control);
...
void BalanceController::calc_A_qpOASES()
{
   ...
   for(int i = 0; i<NUM_CONTACT_POINTS; i++)
   {
      C_control.block<1,3>(5*i+0,3*i) << -mu_friction*direction_normal_flatGround.transpose()
                                    + t1x.transpose();
      C_control.block<1,3>(5*i+1,3*i) << -mu_friction*direction_normal_flatGround.transpose()
                                    + t2y.transpose();
      C_control.block<1,3>(5*i+2,3*i) <<  mu_friction*direction_normal_flatGround.transpose()
                                    + t2y.transpose();                                    
      C_control.block<1,3>(5*i+3,3*i) <<  mu_friction*direction_normal_flatGround.transpose()
                                    + t1x.transpose();                                    
      C_control.block<1,3>(5*i+4,3*i) <<  direction_normal_flatGround.transpose();                                                                   
   }  

   if (use_hard_constraint_pitch==1){
       C_control.row(NUM_CONSTRAINTS_QP-1)=A_control.row(5-1);  // add hard constraint on pitch control
   }
...
void BalanceController::update_log_variables(double *p_des, 
                                          double* p_act,
                                          double* v_des,
                                          double* v_act,
                                          double* O_err)
...
void BalanceController::set_PDgains(double* Kp_COM_in, double* Kd_COM_in, double* Kp_Base_in, double* Kd_Base_in)
{
   Kp_COMx = Kp_COM_in[0];
   Kp_COMy = Kp_COM_in[1];
   Kp_COMz = Kp_COM_in[2];
...
void BalanceController::set_desiredTrajectoryData(double* rpy_des_in, double* p_des_in, double* omegab_des_in, double* v_des_in, double* vdot_des_in)
{
...
{
   std::cout << "\n\n";
   std::cout << "\n\nH = "; print_real_t(H_qpOASES, NUM_VARIABLES_QP, NUM_VARIABLES_QP);
   std::cout << "\n\nA = "; print_real_t(A_qpOASES, NUM_CONSTRAINTS_QP, NUM_VARIABLES_QP);
   std::cout << "\n\ng = "; print_real_t(g_qpOASES, NUM_VARIABLES_QP, 1);
   std::cout << "\n\nlb = "; print_real_t(lb_qpOASES, NUM_VARIABLES_QP, 1);
   std::cout << "\n\nub = "; print_real_t(ub_qpOASES, NUM_VARIABLES_QP, 1);
   std::cout << "\n\nlbA = "; print_real_t(lbA_qpOASES, NUM_CONSTRAINTS_QP, 1);
   std::cout << "\n\nubA = "; print_real_t(ubA_qpOASES, NUM_CONSTRAINTS_QP, 1);
}
...
void BalanceController::copy_Array_to_Eigen(Eigen::VectorXd &target, double* source, int len, int startIndex)
{
   for(int i = 0; i < len; i++)
   {
...
void BalanceController::copy_Array_to_Eigen(Eigen::MatrixXd &target, double* source, int len, int startIndex)
{
   for(int i = 0; i < len; i++)
   {
      target(i) = source[i+startIndex];
