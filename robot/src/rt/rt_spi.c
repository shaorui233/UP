...
cheetahlcm_spi_command_t spi_command_drv;
cheetahlcm_spi_data_t spi_data_drv;
cheetahlcm_spi_torque_t spi_torque;
...
static void
spi_lcm_data_handler(const lcm_recv_buf_t *rbuf, const char *channel, const cheetahlcm_spi_data_t *msg, void *user) {
...
void fake_spine_control(cheetahlcm_spi_command_t *cmd, cheetahlcm_spi_data_t *data, cheetahlcm_spi_torque_t *torque_out,
                        int board_num) {
...
void init_spi(int sim) {
  ...
  // check sizes:
  size_t command_size = sizeof(cheetahlcm_spi_command_t);
  size_t data_size = sizeof(cheetahlcm_spi_data_t);
  ...
  if (sim) {
    ...
    cheetahlcm_spi_data_t_subscribe(lcm_spi, "SIMULATOR_spi_data", &spi_lcm_data_handler, NULL);
...
void spi_to_spine(cheetahlcm_spi_command_t *cmd, spine_cmd_t *spine_cmd, int leg_0) {
  for (int i = 0; i < 2; i++) {
...
void spine_to_spi(cheetahlcm_spi_data_t *data, spine_data_t *spine_data, int leg_0) {
  for (int i = 0; i < 2; i++) {
...
//void increment_driver_status_id(cheetahlcm_spi_data_t* data)
//{
//    data->spi_driver_status = spi_driver_iterations++ << 16;
