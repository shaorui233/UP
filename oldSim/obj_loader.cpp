...
void ObjLoader::load(const char* filename)
{
    ...
    // using aligned memory seems to help performance a bit, at least with the
    // crappy graphics driver
//    posix_memalign((void**)(&positions_jdc),64,Positions.size() * sizeof(float) * 3);
//    posix_memalign((void**)(&normals_jdc),64,Normals.size() * sizeof(float) * 3);

    // malloc works too:
    positions_jdc = (float*)malloc(Positions.size() * sizeof(float) * 3);
    normals_jdc = (float*)malloc(Normals.size() * sizeof(float) * 3);

    for(unsigned int i = 0; i < Positions.size(); i++)
    {
        positions_jdc[i*3 + 0] = Positions[i].x;
        positions_jdc[i*3 + 1] = Positions[i].y;
        positions_jdc[i*3 + 2] = Positions[i].z;
        normals_jdc[i*3 + 0] = Normals[i].x;
        normals_jdc[i*3 + 1] = Normals[i].y;
        normals_jdc[i*3 + 2] = Normals[i].z;
