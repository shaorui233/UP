...
void WalkingOrientation::SaveOptimizationResult(
    ...
    while(curr_time < (_tot_time+dt) ){
        ...
        saveVector(ori, folder+"/"+std::to_string(iter)+"_body_ori", 3, true);
        saveVector(ang_vel, folder+"/"+std::to_string(iter)+"_body_ang_vel", 3, true);
        saveVector(ang_acc, folder+"/"+std::to_string(iter)+"_body_ang_acc", 3, true);
